...
class BM25PlusSquared(BM25Plus):
    """
    Hack thuật toán BM25+: Bình phương chỉ số IDF (IDF^2) 
    để phạt cực nặng từ phổ thông và thưởng điểm từ khóa hiếm.
    """
    def __init__(self, corpus, tokenizer=None, k1=1.5, b=0.75, delta=1.0):
        # 1. Gọi hàm khởi tạo của BM25+ gốc để nó đếm từ và tính IDF bậc 1
        super().__init__(corpus, tokenizer, k1, b, delta)
        
        # 2. Hack: Duyệt qua toàn bộ từ điển IDF và bình phương giá trị của chúng lên!
        for word, idf_value in self.idf.items():
            # Thay vì idf_value, ta dùng (idf_value ** 2)
            self.idf[word] = idf_value ** 2
...
def create_beir_hybrid_retriever(
    ...
    bm25_model = BM25PlusSquared(corpus_tokens)
